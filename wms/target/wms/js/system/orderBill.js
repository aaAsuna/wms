    ...
    $("#edit_table_body").on("click", ".searchproduct", function () {
        ...
        $.dialog.open("/product/selectProductList.do", {
            ...
            close: function () {
                ...
                if (ret) {
                   /* var ids = $.map($("#edit_table_body tr"),function(item) {
                        return $(item).find("[tag=pid]").val();
                    });
                    $.each(ids, function(index,item) {
                        if (ret.id = item) {
                            var num = $("#edit_table_body tr").eq(item).find("[tag=number]").val();
                            $("#edit_table_body tr").eq(item).find("[tag=number]").val(++num)
                        } else {*/
