$(function () {
    ...
    $("#edit_table_body").on("click", ".searchproduct", function () {
        ...
        $.dialog.open("/product/selectProductList.do", {
            ...
            close: function () {
                ...
                if (ret) {
                    ...
                    currentTr.find("[tag=salePrice]").val(ret["salePrice"]);
        ...
        //得到两个元素的值
        var currentTr = $(this).closest("tr");
        var price = currentTr.find("[tag=salePrice]").val() || 0;
        var number = currentTr.find("[tag=salePrice]").val() || 0;
        //设置金额小计
        ...
        $("#editForm").ajaxForm(function (data) {
            if (data.success) {
                $.dialog({
                    ...
                    cancel: true,
                    ok: function () {
                        window.location.href = "/stockOutcomeBill/query.do";
                    }
                });
            }
        });
